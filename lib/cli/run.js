...
const enhanceErrorMessages = require('../util/enhanceErrorMessages.js');
const pck = require('../../package.json');

const requiredNodeVersion = pck.engines.node;
...
// 检测node版本
checkNodeVersion(requiredNodeVersion, '@feitu/cli');

program
  .version(pck.version, '-v, --version') // 版本
...
program
  ...
  .action((templateName, projectName, cmd) => {
    ...
    require('../feitu-create')(lowercase(templateName), projectName);
  });
...
program
  ...
  .action((templateName, gitRepoAddress, cmd) => {
    validateArgsLen(process.argv.length, 5);
    require('../add-template')(lowercase(templateName), gitRepoAddress);
  });

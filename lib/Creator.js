...
module.exports = class Creator extends EventEmitter {
  constructor(name, context) {
    super();
    this.name = name;
    this.context = process.env.FEITU_CLI_CONTEXT = context; // 项目绝对路径
  }

  async create(cliOptions = {}) {
    const { name, context } = this;
    const templateGitRepoJson = readTemplateJson();
    const gitRepoUrl = templateGitRepoJson[process.env.FEITU_CLI_TEMPLATE_NAME];
    console.log('22222222', gitRepoUrl);
    let tmpdir;
    ...
    try {
      ...
      tmpdir = await fetchRemotePreset(gitRepoUrl['download'], true);
    } catch (e) {
      stopSpinner();
